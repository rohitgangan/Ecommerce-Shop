...
public class ProductController {
	...
	@RequestMapping("/view--{categoryId}--product")
	public ModelAndView productsByCategory(@ModelAttribute("product") Product product, @PathVariable("categoryId") int categoryId)
	{
		/*Product pId = productService.getProductById(productId);
		Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().create();
		String pGson = gson.toJson(pId);*/
		Product catId = productService.getBycategory(categoryId);
		Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().create();
		String cGson = gson.toJson(catId);
		ModelAndView model = new ModelAndView("productByCategory");
		model.addObject("listCategory",this.categoryService.listCategory());
		model.addObject("cGson", cGson);
